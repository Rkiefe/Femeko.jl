...
int main()
{
	...
	// Apply the lagrange multiplier technique to extend the stiffness matrix
	// to force a unique scalar potential
	Eigen::VectorXd Lag = Eigen::VectorXd::Zero(mesh.nv);
	for(int k = 0; k<mesh.nt; k++){
		for(int i = 0; i<3; i++){
	    	int nd = mesh.t(i, k);
	    	Lag[nd] += mesh.VE[k]/3.0;
		}
	}

	// Global stiffness matrix (sparse)
	// I'm not going to use 'stiffnessMatrix2D(mesh, Ak, mu)' because
	// I want to extend the matrix to apply the Lagrange multipliers technique
	Eigen::SparseMatrix<double> A(mesh.nv+1, mesh.nv+1);
	{
		std::vector<Eigen::Triplet<double>> triplets;
		triplets.reserve(9 * mesh.nt + 2*mesh.nv);  // At most, the matrix is fully populated
										// (plus the 2 lagrange mult. vectors)
		...
		// Add the lagrange mult. vector
		for(int i = 0; i<mesh.nv; i++){
			// Add the last column (i, mesh.nv)
			triplets.emplace_back(i, mesh.nv, Lag[i]);

			// Add the last row (mesh.nv, i)
			triplets.emplace_back(mesh.nv, i, Lag[i]);
		}
