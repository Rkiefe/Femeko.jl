...
function main(meshSize=0,localSize=0,saveMesh=false)
    ...
    # Generate Mesh
    p,t,surfaceT,InsideNodes,InsideElements = Mesh(cells,meshSize,localSize,saveMesh)

    println("Number of elements ",size(t,2))
    println("Number of nodes ",size(p,2))
    println("Number of surface elements ",size(surfaceT,2))
    println("Number of Inside nodes ",length(InsideNodes))
...
function testCAD(meshSize=0,localSize=0,saveMesh=false)
    ...
    # Import step file
    box = importCAD("STL_Models/cube.step",cells)
